...
var intr = setInterval(function(){ window.location.reload(); }, 900000);
function load_popup_withdata(id,token)
{
	$("#load_accept").attr("src","../upload_outscan_report.php?token="+token+'&id='+id)
	loadPopupBox1('noref',token);
	...
	<div id="templatemo_content" style="height: 600px;">
		...
		<div id="" style="width: 100% !important; " align="center;">
			<div style="width: 100% !important;" align="center;">
				<div align="center"> <label class='text-info h4'>
						...
						$cnt=0;
						$date = date("Y-m-d");
						$query_count_result = "SELECT count(*) FROM appointment_schedule where  date_of_appointment='$date' and mission_id='".$_SESSION['mission_id']."'";
        				...
        				{ 
							...
							$qr_t="SELECT st.token_no as token_no, TIMEDIFF(CURTIME(), st.ao_finish_at) as waiting, a.application_status as application_status,a.id as id  FROM appointment_schedule a, submission_timing st where a.id=st.reference_id and a.date_of_appointment='$date' and st.mission_id='".$_SESSION['mission_id']."' ORDER BY st.token_no  DESC $limit ";
						...
						<tr>
                            ...
                            <?php
							if($recResult['application_status']=='shown_up')
							{
							?>
                            <td align="left">
                            	<input type="button" class="btn btn-info"  onClick="load_popup_withdata('<?php echo $recResult['id']; ?>','<?php echo $recResult['token_no']; ?>')" value="Submit">
                            </td>
                            <td align="left">
                            	<input type="button" class="btn btn-danger"  onClick="reject_ro_other('<?php echo $recResult['token_no']; ?>')" value="Reject">
                           	</td>
                           	<?php
							}
							...
							{
							?>
                            	<td align="left" colspan="2" class="text-info" style="font-size: 15px;">
                                	Bio Pending
                                </td>
                            <?php
							}
        ...
        <div id="popup_box_registration" align="center" style="text-align:center;">
    		<iframe id="load_accept" type="text/html" src="" width="100%" height="100%" style="overflow:auto; border-style: none;" frameborder="0" border="0" cellspacing="0">
    		</iframe>                
