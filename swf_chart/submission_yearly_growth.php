...
include_once("../db_connect.php");
	...
	for($s= 2014; $s<= date('Y'); $s++)
	{
		if($s> 2014)
		{
			$get_totals_this =mysql_query("SELECT ".$s." AS years, COUNT(*) as totals FROM passport_inscan_record WHERE DATE_FORMAT(date_outscan, '%Y') = '".$s."'");
			$totals_result_this = mysql_fetch_array($get_totals_this);
			$this_year_total = $totals_result_this['totals'];
	
			$get_totals_last =mysql_query("SELECT ".($s-1)." AS years, COUNT(*) as totals FROM passport_inscan_record WHERE DATE_FORMAT(date_outscan, '%Y') = '".($s-1)."'");
	...
	<draw>
		...
		<text  transition='dissolve' delay='.5' duration='0.5' color='000033'  size='16' x='220' y='-143' width='700' height='180' h_align='center' v_align='bottom'>ApplicationSubmission - Yearly Growth</text>
	...
	if($fp = fopen("submission_yearly_growth.xml","w"))
	{
