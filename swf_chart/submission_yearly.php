<?php
	...
	$chart_initial = "<chart><license>LTA9-KHS7V.O.945CWK-2XOI1X0-7L</license>
		...
		<axis_value  size='12' color='000000' alpha='100' prefix='' suffix='' decimals='0' separator='' show_min='false' orientation='diagonal_down'/>
	...
	{
		$get_totals =mysql_query("SELECT ".$s." AS years, COUNT(*) as totals FROM passport_inscan_record WHERE DATE_FORMAT(date_outscan, '%Y') = '".$s."'");
		$totals_result = mysql_fetch_array($get_totals);
		$a2 = $a2."<number bevel='data' tooltip='".$s."  Total - Applicants ".$totals_result['totals']."' shadow='low'>".$totals_result['totals']."</number>";
	}
	$row_data = $a1.$a2."</row>";	
	...
	$chart_foorter = "<chart_grid_h alpha='10' thickness='1' type='dashed' />
		...
		<chart_rect bevel='bg' shadow='high' x='225' y='90' width='540' height='200' positive_color='eeeeff' negative_color='dddddd' positive_alpha='100' negative_alpha='100'  corner_tl='0' corner_tr='0' corner_br='0' corner_bl='0' />
	...
	<draw>
		...
		<text  transition='dissolve' delay='.5' duration='0.5' color='000033'  size='16' x='220' y='-143' width='700' height='180' h_align='center' v_align='bottom'>Application Submission Details Year On Year Comparison</text>
		<text  transition='dissolve' delay='.5' duration='0.5' color='000033'  size='16' x='190' y='233' width='300' height='130' h_align='center' v_align='bottom'>Total Applications Accepted</text>
	</draw>
	<filter>
	...
	if($fp = fopen("submission_yearly_bar.xml","w"))
	{
