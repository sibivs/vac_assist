...
if($prname == "all")
{
	...
	for($ij=0; $ij < count($pid_in_array); $ij++)
	{
		...
		for($s= 2014; $s<= date('Y'); $s++)
		{
			if($s> 2014)
			{
				$get_totals_this =mysql_query("SELECT p.service_name AS service_name,  IFNULL(SUM(d.total),0) AS totals FROM price_list p LEFT JOIN daily_vas_entries_total d on p.id=d.vas_id AND YEAR(d.date)='".$s."' WHERE p.id=".$pid_in_array[$ij]."  GROUP BY YEAR(d.date) ORDER BY YEAR(d.date) ASC");
				$totals_result_this = mysql_fetch_array($get_totals_this);
				$this_year_total = $totals_result_this['totals'];
		
				$get_totals_last =mysql_query("SELECT p.service_name AS service_name,  IFNULL(SUM(d.total),0) AS totals FROM price_list p LEFT JOIN daily_vas_entries_total d on p.id=d.vas_id AND YEAR(d.date)='".($s-1)."' WHERE p.id=".$pid_in_array[$ij]."  GROUP BY YEAR(d.date) ORDER BY YEAR(d.date) ASC");
	...
	<draw>
		...
		<text  transition='dissolve' delay='.5' duration='0.5' color='000033'  size='16' x='220' y='-143' width='700' height='180' h_align='center' v_align='bottom'>Value Added Services Year On Year Comparison - ".$totals_result_this['service_name']."</text>
		...
		<text  color='000000' alpha='100' rotation='-90' size='16' x='7' y='430' width='300' height='50' h_align='center' v_align='middle'>Total</text>
			<text  transition='dissolve' delay='.5' duration='0.5' color='000033'  size='16' x='270' y='230' width='300' height='180' h_align='center' v_align='bottom'>Year</text>
	</draw>
	<filter>
...
<tooltip color='000000'
	...
	if($fp = fopen("vas_yearly_growth.xml","w"))
	{
...
else
{
	...
	for($s= 2014; $s<= date('Y'); $s++)
	{
		if($s> 2014)
		{
			$get_totals_this =mysql_query("SELECT p.service_name AS service_name,  IFNULL(SUM(d.total),0) AS totals FROM price_list p LEFT JOIN daily_vas_entries_total d on p.id=d.vas_id AND YEAR(d.date)='".$s."' WHERE p.id=".$prname."  GROUP BY YEAR(d.date) ORDER BY YEAR(d.date) ASC");
			$totals_result_this = mysql_fetch_array($get_totals_this);
			$this_year_total = $totals_result_this['totals'];
	
			$get_totals_last =mysql_query("SELECT p.service_name AS service_name,  IFNULL(SUM(d.total),0) AS totals FROM price_list p LEFT JOIN daily_vas_entries_total d on p.id=d.vas_id AND YEAR(d.date)='".($s-1)."' WHERE p.id=".$prname."  GROUP BY YEAR(d.date) ORDER BY YEAR(d.date) ASC");
			$totals_result_last = mysql_fetch_array($get_totals_last);
			$last_year_total = $totals_result_last['totals'];
			if($last_year_total==0)
			{
