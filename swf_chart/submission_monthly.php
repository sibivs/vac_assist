...
if($year == "all")
{
	...
	{
		$hedings_2=$hedings_2."<string>".$z."-".$yr."</string>";
	}
	$hedings_3= "</row>";
	...
	{
		$get_totals =mysql_query("SELECT '".$s."' AS YEAR,COUNT(*) AS totals FROM passport_inscan_record WHERE  YEAR(date_outscan) = '".$s."' and MONTH(date_outscan)= '".$month."'");
		$totals_result = mysql_fetch_array($get_totals);
		$a2 = $a2."<number bevel='data' tooltip='".$s."  Total - ".$totals_result['totals']."' shadow='low'>".$totals_result['totals']."</number>";
	}
	$row_data = $a1.$a2."</row>";
	...
	<draw>
		...
		<text  color='000000' alpha='100' rotation='-90' size='16' x='100' y='330' width='300' height='50' h_align='center' v_align='middle'>Year</text>
		<text  transition='dissolve' delay='.5' duration='0.5' color='000033'  size='16' x='220' y='-143' width='700' height='180' h_align='center' v_align='bottom'>Application Submission Month On Month Comparison - ".$yr."</text>
	...
	$fp = fopen("submission_monthly_bar.xml","w");
...
else
{
	...
	for($s= 1; $s<= 12; $s++)
	{
		...
		$get_totals =mysql_query("SELECT '".$year."' AS YEAR,COUNT(*) AS totals FROM passport_inscan_record WHERE  YEAR(date_outscan) = '".$year."' and MONTH(date_outscan)= '".$s."'");
			
		$totals_result = mysql_fetch_array($get_totals);
		$a2 = $a2."<number bevel='data' tooltip='".$year." ".$mn_name[$s]." Total - ".$totals_result['totals']."' shadow='low'>".$totals_result['totals']."</number>";
	}
	$row_data = $a1.$a2."</row>";
	$chart_data_last = "</chart_data>";
	$chart_foorter = "<chart_grid_h alpha='10' thickness='1' type='dashed' />
	...
	<draw>
		<text  color='FA051D'  x='920' y='40' size='14'>Toggle Full Screen</text>
		<text  transition='dissolve' delay='.5' duration='0.5' color='000033'  size='12' x='220' y='-123' width='700' height='180' h_align='center' v_align='bottom'>(** Data availability from January 2015 onwards)</text>
		<text  color='000000' alpha='100' rotation='-90' size='16' x='135' y='330' width='300' height='50' h_align='center' v_align='middle'>Month</text>
		<text  transition='dissolve' delay='.5' duration='0.5' color='000033'  size='16' x='220' y='-143' width='700' height='180' h_align='center' v_align='bottom'>Application Submission Month On Month Comparison -".$yr."</text>
