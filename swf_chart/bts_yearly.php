...
$prname = $_REQUEST['pname'];
	//$get_vas_items = mysql_query("select id, service_name from price_list where id='".$prname."' ORDER BY service_name");
	...
	{
		$get_totals =mysql_query("SELECT '".$s."' AS YEAR,  IFNULL(ROUND(SUM((quantity_sold*price_per_item)),1),0) AS totals FROM bts_sales WHERE product_name='".$prname."' and YEAR(date_sold)='".$s."' ORDER BY YEAR(date_sold)");
		$totals_result = mysql_fetch_array($get_totals);
		$a2 = $a2."<number bevel='data' tooltip='".$s." ".$prname."  Total - BD ".$totals_result['totals']."' shadow='low'>".$totals_result['totals']."</number>";
	}
	$row_data = $a1.$a2."</row>";	
	...
	if($fp = fopen("bts_yearly_bar.xml","w"))
	{
